#!/bin/bash
VAR_LOCAL_ENV_OS=CentOS
VAR_LOCAL_ENV_CODENAME=7
...
check_environment() {
  if [ -f "/etc/os-release" ]; then
    ...
    if [ "$baseid" = "$targetid" ] && \
       [ "$VAR_LOCAL_ENV_CODENAME" = "$VERSION_ID" ] && \
...
install_dependencies () {
  ...
  run_without_print yum groupinstall -y "Development Tools"
  if (( $? )); then
    print_color red YUM 필수 의존성 설치에 실패했어요.
    exit 1
  fi

  run_without_print yum install -y wget net-tools dhcp tar iproute
  if (( $? )); then
    print_color red YUM 필수 의존성 설치에 실패했어요.
    exit 1
  fi
...
install_dhcp_server () { 
  run_without_print yum install -y dhcp
  if (( $? )); then
